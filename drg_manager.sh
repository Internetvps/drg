...
check_server_status() {
   echo "Server Status:"
   systemctl is-active $DRG_WEBSOCKET_SERVICE
}
...
restart_websocket_service() {
   echo "Restarting $DRG_WEBSOCKET_SERVICE service..."
   systemctl restart $DRG_WEBSOCKET_SERVICE
   systemctl status $DRG_WEBSOCKET_SERVICE --no-pager  # Optionally, show status after restarting
}

# Function to uninstall proxy script
uninstall_proxy_script() {
   echo "Stopping $DRG_WEBSOCKET_SERVICE service..."
   systemctl stop $DRG_WEBSOCKET_SERVICE

   echo "Disabling $DRG_WEBSOCKET_SERVICE service..."
   systemctl disable $DRG_WEBSOCKET_SERVICE

   echo "Removing Python proxy files..."
   rm -rf "/opt/DRG_websocket"
   rm /usr/local/bin/websocket

   echo "Removing systemd service file..."
   rm -f "/etc/systemd/system/$DRG_WEBSOCKET_SERVICE.service"
...
server_information() {
   ...
   if systemctl is-active --quiet $DRG_WEBSOCKET_SERVICE; then
